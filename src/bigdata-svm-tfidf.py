...
def train_model():
    ...
    clf = SVC()
    ###########网格搜索#############
    C_range = np.array([1,3,5])
    gamma_range = np.array([2,4,6])
    kernel = np.array(['rbf', 'linear'])

    param_grid = dict(gamma=gamma_range, C=C_range,kernel=kernel)
    #cv = StratifiedShuffleSplit(n_splits=10, test_size=0.2, random_state=42)
    gc = GridSearchCV(clf, param_grid=param_grid, cv=2)
    gc.fit(X_train, y_train)
    print("最佳的分类器对应的参数",gc.best_estimator_)

    print("准确率：", gc.best_score_)
    clf = gc.best_estimator_
