...
def TextCNN_model_1(x_train_padded_seqs, y_train, x_test_padded_seqs, y_test):
    ...
    model.fit(x_train_padded_seqs, one_hot_labels, batch_size=500, epochs=3)
...
if __name__ == '__main__':
    dataset = read_csv_file() #获取训练集
    df = train_test()  #测试集和训练集

    tokenizer = Tokenizer()  # 创建一个Tokenizer对象
    # fit_on_texts函数可以将输入的文本中的每个词编号，编号是根据词频的，词频越大，编号越小
    tokenizer.fit_on_texts(df['content'])
    vocab = tokenizer.word_index  # 得到每个词的编号 [0:7340]
    x_train, x_test, y_train, y_test = train_test_split(df['content'][0:50], dataset['label'], test_size=0.1)
